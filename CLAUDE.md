...
- **Tool-neutral language in governance docs.** Documents `01`–`03` and `07` (charter, roadmap, amendment process, bylaws) say "the public forum" and "the public record," never "GitHub" or "git repository." The one place that binds terms to concrete tools is `tooling.md`. Don't leak tool names into the governance docs.
...
- **Changing governing documents** (charter, bylaws, roadmap, standing processes) goes through the pollution → solution pipeline in `03-amendment-process.md` and lands in `amendment-log.md`. Editorial fixes to draft docs are normal work; substantive changes to reviewed docs are amendments.
...
`.github/ISSUE_TEMPLATE/pollution.yml` mirrors the required pollution fields from `03-amendment-process.md` (What's polluted / Who it affects / Cost of not fixing / Acceptance criteria + optional solution sketch). If the amendment process's required fields change, update the template to match.
